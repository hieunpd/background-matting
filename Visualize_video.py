...
def evaluateModelCV(model, mean, std, imgW, imgH, Lovasz):
    # gloabl mean and std values


    syn_bg = cv2.imread('./beach.jpg')
    ...
    while (success):
        ...
        # seg_img = cv2.resize(seg_img,(720, 720))
        FPS = int(1.0 / (time.time() - start_time))
        cv2.putText(seg_img, f'FPS: {str(FPS)}', (10,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        cv2.imshow("capture", seg_img)
...
def evaluateModelPIL(model, mean, std, imgW, imgH, Lovasz):
    # gloabl mean and std values
...
def ExportVideo(model, Maxfile, savedir, model_name,  videoName,h,w , mean, std, Lovasz, pil=True):
    ...
    if pil:
        evaluateModelPIL(model, mean, std, w, h, Lovasz)
...
if __name__ == '__main__':
    ...
    Max_name = "./result/SINet/SINet.pth"
    ...
    if Lovasz:
        train_config["num_classes"] = train_config["num_classes"]
    ...
    evaluateModelCV(model, mean, std, data_config["w"], data_config["h"], Lovasz)
